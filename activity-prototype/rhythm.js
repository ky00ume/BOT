...
function pluckLane(lane,hold=false){const ctx=audioCtx();if(!ctx)return;stopLane(lane,.025);const o=ctx.createOscillator(),g=ctx.createGain(),f=ctx.createBiquadFilter(),t=ctx.currentTime;o.type=SONG.instrument==='LUTE'?'sawtooth':'triangle';o.frequency.value=LANE_FREQ[lane];f.type='lowpass';f.frequency.value=SONG.instrument==='LUTE'?1450:2400;g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(.065,t+.008);if(hold){g.gain.exponentialRampToValueAtTime(.025,t+.18)}else{g.gain.exponentialRampToValueAtTime(.0001,t+.32)}o.connect(f).connect(g).connect(ctx.destination);o.start(t);if(hold){heldVoices.set(lane,{o,g,ctx})}else{o.stop(t+.34)}}
