...
const pressedKeys=new Set(),activeHolds=new Map(),travel=1800,windows={perfect:45,great:85,good:130};let spaceNotes=[],spaceActive=null;
...
function flash(t,c){els.judge.textContent=t;els.judge.className='judge '+c;els.judge.classList.remove('pop');void els.judge.offsetWidth;els.judge.classList.add('pop');clearTimeout(flash._t);flash._t=setTimeout(()=>els.judge.classList.remove('pop'),170)}
function burstHit(lane,cls){if(lane===null||lane===undefined)return;const laneEl=els.lanes.children[lane];if(!laneEl)return;const b=document.createElement('i');b.className='hit-burst '+cls;b.innerHTML='<span></span><span></span><span></span>';laneEl.appendChild(b);setTimeout(()=>b.remove(),260)}
function now(){return running&&Number.isFinite(els.song.currentTime)?els.song.currentTime*1000:performance.now()-startAt}
...
function pulseHit(lane,cls='perfect'){const r=els.stage.querySelectorAll('.receptor')[lane];if(!r)return;r.classList.remove('hit-perfect','hit-great','hit-good');void r.offsetWidth;r.classList.add('hit-'+cls);setTimeout(()=>r.classList.remove('hit-'+cls),150)}
function award(err,lane=null){adapt(true);const result=grade(err);if(lane!==null){pulseHit(lane,result.cls);burstHit(lane,result.cls)}judged++;totalErr+=err;combo++;counts[result.cls]++;maxCombo=Math.max(maxCombo,combo);score+=result.pts+Math.min(combo,100)*5;flash(result.text,result.cls);if(mode==='hard'&&result.cls==='perfect'&&runes<4&&combo%6===0){runes++;renderRunes();if(runes===4){flash('RUNE AWAKENED','perfect');els.stage.classList.add('awakened')}}updateHud()}
...
function frame(){if(!running)return;const t=now(),h=els.stage.clientHeight,judgeY=els.stage.querySelector('.judgeline')?.offsetTop ?? h-parseFloat(getComputedStyle(els.stage).getPropertyValue('--receptor-y')||72);
 spaceNotes.forEach(n=>{if(n.miss)return;if(n.holding){if(n.el){const remaining=Math.max(0,n.t+n.duration-t),height=Math.max(14,remaining/travel*(judgeY-20));n.el.style.height=`${height}px`;n.el.style.top=`${judgeY-height}px`;n.el.classList.add('holding')}return}if(n.hit)return;const dt=n.t-t;if(dt < -windows.good){n.miss=true;miss('PEDAL MISS');n.el?.remove();return}if(dt>travel)return;if(!n.el){n.el=document.createElement('div');n.el.className='space-note';els.stage.appendChild(n.el)}const y=judgeY-(dt/travel)*(judgeY-20),height=Math.max(14,n.duration/travel*(judgeY-20));n.el.style.height=`${height}px`;n.el.style.top=`${y-height+14}px`;});updateLyrics(t);phaseUpdate(t);chart.forEach(n=>{
 ...
 });if(els.song.ended||t>=Math.max(0,(Number.isFinite(els.song.duration)?els.song.duration*1000:56000)-40)){finish();return}raf=requestAnimationFrame(frame)}
function pressSpace(){if(!running||mode!=='hard')return;const t=now();let best=null,err=Infinity;spaceNotes.forEach(n=>{if(n.hit||n.miss)return;const e=Math.abs(n.t-t);if(e<err){err=e;best=n}});if(!best||err>windows.good+assist*45){combo=0;flash('PEDAL MISS','miss');updateHud();return}best.hit=true;best.holding=true;spaceActive=best;if(!best.el){best.el=document.createElement('div');best.el.className='space-note';els.stage.appendChild(best.el);const h=els.stage.clientHeight,judgeY=els.stage.querySelector('.judgeline')?.offsetTop ?? h-parseFloat(getComputedStyle(els.stage).getPropertyValue('--receptor-y')||72),height=Math.max(14,best.duration/travel*(judgeY-20));best.el.style.height=`${height}px`;best.el.style.top=`${judgeY-height}px`}best.el.classList.add('hit','holding');award(err);els.stage.querySelector('.space-receptor')?.classList.add('active');els.stage.classList.remove('pedal-hit');void els.stage.offsetWidth;els.stage.classList.add('pedal-hit');setTimeout(()=>els.stage.classList.remove('pedal-hit'),180);}
function releaseSpace(){els.stage.querySelector('.space-receptor')?.classList.remove('active');if(!spaceActive)return;const n=spaceActive;spaceActive=null;n.holding=false;const err=Math.abs(now()-(n.t+n.duration));if(err<=windows.good+assist*45){award(err);flash('PEDAL RELEASE','perfect')}else miss('PEDAL RELEASE');if(n.el){n.el.classList.remove('holding');n.el.classList.add('hold-complete');setTimeout(()=>n.el?.remove(),120)}}
function press(key){if(!running)return;const lane=MODES[mode].keys.indexOf(key);if(lane<0)return;pressedKeys.add(key);els.lanes.children[lane].classList.add('pressed');els.stage.querySelector('.receptors')?.children[lane]?.classList.add('active');const t=now();
 ...
 best.hit=true;hits++;pluckLane(lane,!!best.duration);award(err,lane);if(best.duration){best.holdState='holding';activeHolds.set(lane,best);flash(best.charge?'CHARGE':best.releaseOnly?'RELEASE HOLD':'HOLD','great')}if(best.el){best.el.classList.add('hit');if(!best.duration)setTimeout(()=>best.el?.remove(),130)}
}
function release(key){const lane=MODES[mode].keys.indexOf(key);if(lane<0)return;pressedKeys.delete(key);els.lanes.children[lane]?.classList.remove('pressed');els.stage.querySelector('.receptors')?.children[lane]?.classList.remove('active');const note=activeHolds.get(lane);if(!note)return;activeHolds.delete(lane);stopLane(lane);if(note.holdState!=='holding')return;const err=Math.abs(now()-(note.t+note.duration));if(err<=windows.good){note.holdState='complete';award(err,lane);if(note.releaseOnly)flash('RELEASE','perfect');else if(note.charge)flash('CHARGE RELEASE','perfect');if(note.el)note.el.classList.add('hold-complete')}else{note.holdState='failed';miss('HOLD MISS')}}
async function start(){reset();els.stage.focus();const drill=mode==='hard'&&new URLSearchParams(location.search).get('drill')==='1';els.status.textContent=drill?'EXPERT · 1분 패턴 테스트곡':`${MODES[mode].label} · ${mode==='hard'?'화음·홀드·즉흥 기억/응답 활성':'정석 연주'}`;for(const n of ['3','2','1','START']){els.countdown.textContent=n;els.countdown.classList.toggle('go',n==='START');await new Promise(r=>setTimeout(r,n==='START'?320:500))}els.countdown.textContent='';els.countdown.classList.remove('go');try{els.song.playbackRate=playbackSpeed();await els.song.play()}catch(e){els.status.textContent='오디오 재생을 시작하지 못했습니다. 다시 눌러 주세요.';return}startAt=performance.now()-els.song.currentTime*1000;running=true;document.body.classList.add('playing');raf=requestAnimationFrame(frame)}
...
els.start.addEventListener('click',start);els.resultAgain.addEventListener('click',start);document.addEventListener('keydown',e=>{const k=e.key.toLowerCase();if(e.code==='Space'&&mode==='hard'){e.preventDefault();if(!e.repeat)pressSpace();return}if(MODES[mode].keys.includes(k)){e.preventDefault();if(!e.repeat)press(k)}});document.addEventListener('keyup',e=>{if(e.code==='Space'){e.preventDefault();releaseSpace();return}release(e.key.toLowerCase())});
reset();window.rhythmDemo={pressSpace,releaseSpace,clockNow:now,getState:()=>({mode,phase,score,combo,hits,judged,runes,memory:memory.length,duration:chart.length?chart.at(-1).t+900:0,notes:chart.map(n=>({t:n.t,lane:n.lane,type:n.type,duration:n.duration,releaseOnly:n.releaseOnly,charge:n.charge,hit:n.hit,holdState:n.holdState})),spaceNotes:spaceNotes.map(n=>({t:n.t,duration:n.duration,hit:n.hit,miss:n.miss}))}),press,release,Patterns};
})();
