...
class BattleEngine:
    ...
    def start_encounter(self, zone_name: str = None) -> tuple:
        ...
        # 전투 시작 이미지 렌더링 (build_battle_image 활용)
    ...
    def process_turn(self, skill_id: str = "smash") -> io.BytesIO:
        ...
        if self.monster_hp <= 0:
            ...
            rows = [
                {"label": "공격", "value": f"{'크리티컬! ' if crit else ''}{monster['name']}에게 {dmg} 피해"},
        ...
        buf = get_renderer().render_battle_card(
            ...
            last_action=f"{'💥크리티컬! ' if crit else ''}{skill_name} → {monster['name']}에게 {dmg} 피해  |  반격 -{mon_dmg}HP",
            last_dmg=dmg,
            is_crit=crit,
