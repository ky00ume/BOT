...
_DIARY_TEMPLATES = [
    ...
    (
        ...
        3,
        lambda stats: (
            f"오늘 전투를 {stats.get('battles_won', 0)}번이나 했슴미댜!! "
            "이제 슬슬 전투에 익숙해지는 것 같슴미댜. 강해지고 있는 느낌! ⚔️✨"
        ),
    ),
    (
        "fish_caught",
    ...
    (
        ...
        3,
        lambda stats: (
            f"오늘 물고기를 {stats.get('fish_caught', 0)}마리나 잡았슴미댜! "
            "강가가 참 좋슴미댜~ 물소리도 좋고 바람도 시원하고... 🎣🌊"
        ),
    ),
    (
        "gold_earned",
    ...
    (
        ...
        100,
        lambda stats: (
            f"오늘 무려 {stats.get('gold_earned', 0)}골드를 벌었슴미댜!! "
            "이 정도면 맛있는 거 사먹어도 되겠쥬?! 헤헤~ 💰💰"
        ),
    ),
    (
        "items_cooked",
    ...
    (
        ...
        3,
        lambda stats: (
            f"오늘 요리를 {stats.get('items_cooked', 0)}번이나 했슴미댜! "
            "데리스 본클록 아줌마가 솜씨가 늘었다고 칭찬해줬슴미댜~~ 헤헤 🍳✨"
        ),
    ),
    (
        "pet_count",
    ...
    (
        ...
        1,
        lambda stats: (
            f"오늘 퀘스트를 {stats.get('quests_done', 0)}개 완료했슴미댜! "
            "마을 사람들이 고마워하는 모습을 보면 뿌듯하슴미댜~ 📋✨"
        ),
    ),
    (
        "items_gathered",
        1,
        lambda stats: (
            f"채집을 {stats.get('items_gathered', 0)}번 했슴미댜! "
            "자연에서 얻은 재료들이 반짝반짝 빛나슴미댜~ 🌿"
        ),
    ),
    (
        "npc_talks",
        1,
        lambda stats: (
            f"마을 사람들과 {stats.get('npc_talks', 0)}번 대화했슴미댜! "
            "다들 재미있는 이야기를 많이 해주셨슴미댜~ 💬"
        ),
    ),
]
...
_DEFAULT_DIARY_ENTRIES = [
    ...
    "오늘은 비가 왔슴미댜. 거미줄에 맺힌 빗방울이 보석 같았슴미댜... 🌧️✨",
    "오늘은 마을 광장에서 버나드 언니가 노래하는 걸 들었슴미댜. 너무 아름다웠슴미댜... 🎵",
    "오늘은 블러그이랑 같이 짐을 날랐슴미댜. 힘들었지만 간식을 줘서 기분이 좋았슴미댜~ 🍪",
    "오늘은 밤하늘의 별을 세었슴미댜. 47개까지 세다가 잠들어 버렸슴미댜... ⭐💤",
    "오늘은 데리스 본클록 아저씨 대장간에서 따뜻한 차를 마셨슴미댜. 불꽃 소리가 편안했슴미댜 ☕🔥",
    "오늘은 오멜룸랑 마법 연습을 했슴미댜. 아직 어렵지만 재밌슴미댜! ✨🔮",
    "오늘은 군주 스포 아저씨랑 마을 순찰을 했슴미댜. 마을이 안전해서 다행임미댜! 🛡️",
    "오늘은 바람이 많이 불었슴미댜. 거미줄이 날아갈 뻔했슴미댜...ㅠㅠ 🍃",
    "오늘은 맛있는 수프를 만들어 먹었슴미댜! 데리스 본클록가 가르쳐준 대로 했더니 잘 됐슴미댜~ 🍲",
    "오늘은 숲에서 예쁜 나비를 봤슴미댜. 나비가 머리 위에 앉았슴미댜!! 🦋💕",
]
