...
EVENTS = {
 "셀루네 수정지": {"id":"selune_sussur_bloom","title":"🌙 마법을 삼키는 푸른 꽃","text":"달빛을 머금은 수정 사이에서 주변의 마력을 잠재우는 푸른 꽃 한 송이가 흔들립니다.","item":"sussur_bloom","item_name":"수서꽃","choices":[("study","🔎 마력의 흐름을 읽는다"),("take","🧤 조심히 꺾는다"),("leave","🚶 그대로 둔다")]},
...
def resolve(player, zone, choice, *, rng):
    ...
    if rng()<chance:
        if player.add_item(e['item'],1):
            st.update(resolved=True,outcome="found")
