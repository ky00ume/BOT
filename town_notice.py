...
def make_npc_embed() -> discord.Embed:
    ...
    npcs = [
        ("데리스 본클록",       "대장장이",     "무기·방어구 상점 / 단조 보조 알바"),
        ("오멜룸",     "약초상",       "소모품·도구 상점 / 약초 채집 알바"),
        ("블러그",         "상인",         "가방 상점 / 짐 운반 알바"),
        ("오멜룸",   "마법사",       "마법 연구소 / 실험 보조 알바"),
        ("군주 스포",   "경비대장",     "마을 성문 / 순찰 보조 알바"),
        ("데리스 본클록",     "요리사",       "마을 식당 / 주방 보조 알바"),
        ("툴라",       "낚시꾼",       "강가 / 낚시 보조 알바"),
        ("버나드",     "음유시인",     "마을 광장 / 공연 보조 알바"),
        ("군주 스포",   "길드 마스터",  "모험가 길드 / 길드 업무 알바"),
        ("블러그","마법학교 교수","마법학교 / 수업 보조 알바"),
        ("글럿",     "교관",         "훈련소 / 훈련 보조 알바 / 수련"),
        ("버나드",       "여관 주인",    "비전 타운 여관 / 휴식·회복"),
    ]
...
def make_life_embed() -> discord.Embed:
    ...
    embed.add_field(
        name="🏨 여관 & 🎵 연주",
        value=(
            "버나드 NPC 대화 → **[휴식]** 버튼 — 기력·HP 회복\n"
            "  └ 간이 휴식(50G) / 숙박(150G) / 특실(300G)\n"
            "버나드 NPC 대화 → **[연주]** 버튼 — 연주 스킬 훈련\n"
            "  └ 연주 스킬북: 데리스 본클록/버나드 상점에서 구매"
        ),
        inline=True,
    )
...
def make_patchnote_v053_embed() -> discord.Embed:
    ...
    embed.add_field(
        ...
        value=(
            ...
            "• 버나드 꽃 채집 알바: 채집 시 알바 진행도 미반영 수정\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "• `/수련` 삭제 → 글럿 NPC 대화 **[수련]** 버튼으로 이동\n"
            "• `/연주` 삭제 → 버나드 NPC 대화 **[연주]** 버튼으로 이동\n"
