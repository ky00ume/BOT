...
SPECIAL_NPC_RESPONSES = {
    ...
    "카르니스": {
        "마제스티": "\"마제스티는 조용하다. 좋다. 조용한 것은... 거짓말을 덜 하지. 가까이 오게 두어라.\"",
        "문랜턴": "카르니스의 손가락이 등불의 금속 테두리를 감싼다. \"우리 여인의 빛이다. 가까이 있어라. 안에서는 늘 목소리가 난다... 하나는 특히 시끄럽지.\"",
        "그림자": "\"그림자는 굶주렸다. 뒤처지는 것은 먹이가 된다. 우리는 먹이지 않는다. 따라와.\"",
        "절대자": "그가 잠시 고개를 기울인다. \"그분이 우리를 보았다. 버려진 우리를. 그분의 빛이 길을 주었다.\"",
        "롤스": "여덟 다리가 돌바닥을 긁는다. \"그 이름은 말하지 마라. 버린 여왕이다. 지웠다. 잊었다. ...우리는 살아남았다.\"",
    },
    "루바토": {
...
class SpecialNPCView(View):
    ...
    async def _buff_callback(self, interaction: discord.Interaction):
        ...
        if self.npc_name == "루바토":
            ...
            msg = (
                f"🕷️ **카르니스**가 문랜턴의 빛 안쪽으로 당신을 끌어당긴다. \"빛 안에 있어라. 아직 쓰러질 때가 아니다.\"\n"
    ...
    async def _gift_callback(self, interaction: discord.Interaction):
        ...
        async def gift_select_cb(sel_interaction: discord.Interaction):
            ...
            gift_responses = {
                ...
                "카르니스": f"카르니스가 {item_name}을(를) 한참 들여다본다. \"...두고 가라. 우리가 기억하겠다.\"",
