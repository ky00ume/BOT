...
SPECIAL_NPC_KEYWORDS = {
    "라파엘": ["계약", "거래", "보상", "소문", "악마"],
    "카르니스": ["마제스티", "하이네스", "츄라이더", "그림자등불", "분화"],
    "루바토": ["카르니스", "리라", "노래", "모험", "그림자등불"],
}
...
SPECIAL_NPC_RESPONSES = {
    ...
    "카르니스": {
        "마제스티": "여덟 눈이 가늘어진다. \"마제스티께서 널 아끼신다. 끝없이 쓰다듬고, 곁에 두고 싶어 하시지. ...그러니 내가 참는 거다.\"",
        "하이네스": "\"네 하이네스가 널 얼마나 감싸는지는 안다. 그 손이 널 놓지 않으니 나도 손대지 않는 거다. ...그 이상으로 착각하지 마.\"",
        "츄라이더": "다리 끝이 바닥을 날카롭게 긁는다. \"나에게서 떨어져 나온 주제에 내 앞을 돌아다녀? ...사라져. 내가 마음을 바꾸기 전에.\"",
        "그림자등불": "카르니스가 섀도우랜턴을 끌어안듯 가까이 당긴다. \"그림자등불은 우리와 함께 간다. 함부로 손대지 마라.\"",
        "분화": "\"그건 탄생이 아니야. 떨어져 나간 거다. 내 것이었던 것이 제멋대로 모양을 얻었지. ...그러니 보기 싫다.\"",
    },
    "루바토": {
        "카르니스": "루바토가 리라 줄을 가볍게 튕긴다. \"카르니스? 무섭긴 하지. 그래도 그 사람한테도 들리는 노래가 있어. 나는 그걸 알아.\"",
        "리라": "\"이건 그냥 악기가 아니야. 말로 못 하는 건 얘한테 맡기면 돼. 이상하지? 현 몇 줄이 사람보다 솔직할 때가 있다니까.\"",
        "노래": "\"노래는 남아. 사람이 떠나도, 길이 끝나도. 그러니까 부를 수 있을 때 많이 불러두는 거야.\"",
        "모험": "\"위험한 건 질색인데 말이지, 가만히 있으면 노래가 안 생겨. 그러니 이번에도 무사히 돌아오는 걸 목표로 하자!\"",
        "그림자등불": "루바토가 잠시 웃음을 거둔다. \"그림자등불... 카르니스의 것이고, 카르니스의 길이기도 해. 함부로 만지지는 마.\"",
    },
}
...
class SpecialNPCView(View):
    ...
    async def _buff_callback(self, interaction: discord.Interaction):
        ...
        if self.npc_name == "루바토":
            ...
            msg = (
                f"🕷️ **카르니스**가 당신을 거칠게 밀어 섀도우랜턴(그림자등불)의 범위 안에 둔다. \"죽지 마라. 마제스티가 슬퍼하니까. ...그뿐이다.\"\n"
    ...
    async def _gift_callback(self, interaction: discord.Interaction):
        ...
        async def gift_select_cb(sel_interaction: discord.Interaction):
            ...
            gift_responses = {
                ...
                "카르니스": f"카르니스가 {item_name}을(를) 받아 들고도 표정을 풀지 않는다. \"마제스티가 예의를 지키라 했지. ...받아두겠다. 가까이 오진 마.\"",
